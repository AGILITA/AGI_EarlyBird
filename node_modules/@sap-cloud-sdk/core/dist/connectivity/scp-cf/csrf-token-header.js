...
function makeCsrfRequest(destination, requestConfig) {
    ...
    return http_client_1.executeHttpRequest(destination, appendSlash(axiosConfig))
        .then(function (response) { return response.headers; })
        .catch(function (e1) {
        logger.error(new util_1.ErrorWithCause('Initial try to fetch CSRF token failed - retry without slash at ', e1));
        return http_client_1.executeHttpRequest(destination, removeSlash(axiosConfig))
            .then(function (response) { return response.headers; })
            .catch(function (e2) {
            throw new util_1.ErrorWithCause('Also second try to fetch SRF token failed - No CSRF token fetched.', e2);
        });
    });
}
...
function removeSlash(requestConfig) {
    if (requestConfig.url.endsWith('/')) {
        requestConfig.url = remove_slashes_1.removeTrailingSlashes(requestConfig.url);
    }
    return requestConfig;
}
